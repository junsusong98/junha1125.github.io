...
3. 정말 많은 노력이 보였다. 내가 과연 이정도를 할 수 있을까? 라는 생각이 들었지만 딱 한달 동안 이와 관련된 매일 논문 1개씩 읽는다면, 잘하면 좀더 창의력과 실험을 가미해서 더 높은 성능을 내는 모델을 만들 수 있지 않을까? 하는 생각은 든다. 
...
5. Boundary Learning - (CASENet, InstanceCut- old) Zimmermann et al.[47]
...
5. Zimmermann et al.[47] - Faster training of mask r-cnn by focusing - 2019
...
- this Network **learn a global boundary representation**. 많은 문제점을 가진 global-mask-based methods 보다 좋다.
- new unified **quality** **measure** of both mask and boundary. new **score** for the per-instance predictions.
-  Regardless of being **single-stage or two-stage** frameworks,  **state-of-the art** methods on the COCO dataset.



# 1. Introduction

- most instance segmentation 의 문제점 : 먼저 생긴 bounding box에 의존된다(step-wise).  성능 구림(region-specific information and ROI-pooled features 때문에) 이 두가지 문제를 아래의 것들로 해결했다.
- **basis framework** : **global image-level** information 을 결합해서 사용. 하지만 정확한 a supervised way로 e global mask representations를 학습하진 않는다. last mask output signal에 의해서 학습 되므로(?)
- **Boundary-preserving Mask RCNN-2020** 
...
- a boundary prediction head + the recent **single-stage** instance segmentation = **the RoI-wise head of Boundary-preserving** 
  - **holistic image-level** instance boundaries(global mast representations) be learned -> distinct advantages
- Use **the boundary ground-truths** 
- Learn/Use **a novel instance-wise boundary-aware mask score** in test-time
- **To sum up**
  ...
  - BlendMask(strongest single-stage instance segmentation methods)를 보완하여 만듬
...
- Single-stage 
  ...
  - **global-area-based** methods 탄생 : (1)  Generate intermediate FCN feature maps, called ‘basis’. (2)  Assemble the extracted basis features.
  - **3.background** 에서 더 다룰 예정.
...
- 위의 pipeline을 따르는 모델들
  1. **YOLACT** :  32 global bases, the according instance-specific scalar coefficients, a linear combination among the bases.  ---> develop \[rich instance features **vs** effective assembly methods\]
  2. improved the assembly parameters : **BlendMask** [6], SipMask [5], and **CenterMask** [38] 
  3. instance-specific representation : **CondInst** [37] (Key : dynamic filter weights, sliding window, the predicted convolution weight filters)
- our step
  ...
  3. **a boundary-aware mask score** (about the mask and boundary quality)
...
<img src="C:\Users\sb020\AppData\Roaming\Typora\typora-user-images\image-20210119165307132.png" alt="image-20210119165307132" style="zoom:80%;" />
...
- Learning **holistic image boundary**
  ...
  2. image boundary(Missing piece 그림에 없음)
     ...
     - Overlapping objects and complex shapes 에서 좋은 성능 나옴.
  ...
  5. Detection Head
     ...
     - 특히 attention map은 basic head의 결과와 결합되어 boundary basic이 이뤄진다.
- Boundary-aware mask scoring
  ...
  5. Boundary score (S_boundary 정의)
     ...
     - <img src="C:\Users\sb020\AppData\Roaming\Typora\typora-user-images\image-20210119181451921.png" alt="image-20210119181451921" style="zoom:80%;" />
  ...
  7. Score definition at inference
     -  <img src="C:\Users\sb020\AppData\Roaming\Typora\typora-user-images\image-20210119183330139.png" alt="image-20210119183330139" style="zoom:80%;" />
...
- Basis head design choices
  - <img src="C:\Users\sb020\AppData\Roaming\Typora\typora-user-images\image-20210119184502140.png" alt="image-20210119184502140" style="zoom:67%;" />
