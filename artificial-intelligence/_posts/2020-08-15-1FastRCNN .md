---
layout: post
title: 【Paper ,Vision】 Faster RCNN 개념, OpenCV Detection 구현
description: >
...
- 위 이미지에 대해서 오른쪽 Feature Map의 하나의 1x1x512 pixel 값은 왼쪽 이미지의 16x16x3 pixel이 가지는 특징이 무엇인지에 대한 정보를 가지고 있다.  
...
- 참고로 아래의 x y w h 는 **대강** 추천해주는 Region Proposal 결과이다. 

 - RPN은 간단하다. Featrue Map에 (1 padding + 3x3 conv)  + (1x1 conv)를 하여 Classification, Box Regression 정보를 얻을 수 있다.

 - Softmax Classification에서 1*1*512 필터 하나가 하나의 엥커에 의해서 판단되는 결과(객체 유vs무)가 나와야 한다. 이 객체 유무의 값에서 '유'값(0~1값이) RPN에서의 Confidence값이라고 생각하면 된다. 

 - Bounding Box Regression에서 1*1*512 필터 하나가 하나의 엥커에 의해서 판단되는 결과(대강 물체 Region Proposal)가 나와야 한다. 

 - 참고 이미지 (알파프로젝트 하는 동안 Faster RCNN공부한 내용) 

<p align="center"><img src='https://user-images.githubusercontent.com/46951365/91658806-74cacb80-eb06-11ea-804e-566f635883bd.png' alt='drawing' width='800'/></p>
...
- 학습순서
    ...
    2. Ground True랑 방금 올려놓은 Anchor 박스와의 Positive, Negative 정도를 판단한다. 
    3. Ground True랑 방금 올려놓은 Anchor 박스와의 Bounding Box Regression 정도를 판단한다. (Anchor에 대해서, 어느정도로 이동하고 확장해야, Ground True Box로 근접하여 positive가 될 수 있는지 판단한다. (Anchore이동으로 완벽히 Ground True가 될 필요는 없다. Anchor이동으로 Ground True에 대해서 Positive가 되기만 하면 된다. ))
...
- 만약 모든 Positive Anchor가 Proposal된 Region이라고 한다면 너무 많을 수 있으므로, 다음과 같은 절차를 수행한다. 
    - Objectness Score를 계산 한다.  
      Objectness Score = pi값(Object일 확률 softmax값) * Ground True와 Bounding Box갑의 IOU값
