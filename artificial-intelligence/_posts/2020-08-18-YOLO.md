...
- Output
     ...
     - 1개의 Anchor box에 대해서 25개의 정보를 가진다. 4(bounding box regression) + 1(confidence) + 20(class softmax)
...
- Ouput
     - Output된 결과가 Box 하나당 4(bounding box regression) + 1(confidence) + 80(class Logistic 값)이다. 
...
- Location Prediction(아래 사진)
          ...
          - c는 Anchor 박스의 좌표
          - p는 Anchor 박스의 크기
          - t는 offset 값 = 이게 Inference에서 최종 Output으로 나와야 한다.  
          - 좌표 offset으로 0~1사이 값인 시그모이드 값 사용. Cell의 중심으로 부터 bounding box가 너무 옮겨지지 않도록 해야한다. 
          - 시그모이드 값으로는 = object이냐 아니냐 확률 x GT와 나의 최종 예측 bounding box와의 IOU 이 나와야 한다. 
