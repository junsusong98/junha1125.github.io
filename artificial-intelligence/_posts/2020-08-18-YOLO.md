...
- Location Prediction(아래 사진)  
     ...
     - 시그모이드 값으로는 = object이냐 아니냐 확률 x GT와 나의 최종 예측 boundingbox와의 IOU 이 나와야 한다. 
