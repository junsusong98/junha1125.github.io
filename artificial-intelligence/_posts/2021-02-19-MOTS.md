...
- **느낀점** :
  ...
  - 그들의 성능평가 matrix 자체 점수를, 조금이라도 올리기가 매우 힘들다면 모를까.. 확실히 아직은 MOTS 에 대해서 더 많은 연구가 이뤄져야 할 듯 하다.
...
2. **Integrating temporal context**
   -  depthwise separable 3D convolution layers with 3×3 (spatial) ×3 (time) filter kernels
   - ResNet-101
   - 위의 사진에서는 3D conv가 들어가지만, 대체신경망으로 (the spatial structure 를 유지하고 있는) Convolutional LSTM (+  skip connection)  을 사용하는 것도 고려해 보았다고 한다. 논문에서 `Convolutional LSTM`를 검색하면, 이것을 사용할 때의 장점, 효과, 실험과정, 에 대해서 간단히 살펴볼 수 있다.
3. **Association Head**
...
5. **Tracking**
   ...
   - 이 부분에서 tracking system  (δ, β, γ)를 찾기 위해 random search를 사용했다고 한다.
...
- **Main Results**
  ...
  - 맨위의 TrackR-CNN(box orig)는 mask head 가 없는 TracR-CNN이다. 
  - +MG : segmentation masks as a post-processing step. 즉 Mask R-CNN의  the mask head 이라고 한다.
- Compare
  - CAMOT [43] : stereo 정보를 사용
  - 코드 사용이 가능한 mask-based trackers 가 없다. 대신 the bounding box-based tracker인 CIWT [42] and BeyondPixels [53]에 fine-tunning된 Mask R-CNN의  the mask head를 결합한 모델을 사용해 보았다.
- Temporal Component
  - backbone feature map과 RPN 사이의 2xConv3D (two depthwise separable 3D convolutional layers)를 넣었을 때의 장점
