...
- **느낀점** : 
  ...
  - Method를 다 읽고 생각해봐도, 뭔지 알지도 못하면서 그냥 배경지식으로 나혼자 짜맞춰서 Abstract, Conclusion, Instruction을 정리하는 것은 무식한 것 같다.
...
- 나중에 필요하면 읽자... ㅎㅎ
- 이 논문의 Relative work는 `5.2. Comparisons with state-of-the-art methods`부분에 짧게 존재한다.
...
- 위에 아래의 내용들을 순서대로 정리. 나중에 참고 할 것.
  1. denoting(변수정의)
  2. self-supervised Domain adaptation에서 Loss함수 
...
1. **formula to rectify nosiy pseudo label** 
   ...
   - 해결책 2 수행 방법 : fix the soft pseudo labels & progressively weight(해주다) them by class-wise probabilities using Centroids(Prototypes)  (주의!) 이때, 아래의 수식에서 P_t,o는 처음 source model에 의해서 정해진 값이고 학습과정 내내 고정되어 있다고 한다.(코드 확인하기)      
...
2. **weight calculate**
   ...
   - 아래 공식의 ω_t는 직관적으로 p_t의 비슷할테지만, 실험적으로 trust confidence(label)과 비슷한 것을 확인할 수 있다. 이 weight공식은 few-shot learning에서 많이 사용되는 기술이다. few-shot learning에서는 classifying new samples을 목적으로 사용하지만 우리는 당연히 recity pseudo labels을 하기 위에서 사용한다.       
...
3. **Prototype(centroids) computation**
   ...
   - 이 값은 on-the-fly으로 계산되고 업데이트 된다. 이 값을 계산하는 것은 ` computational-intensive` (계산 집약된 무거운) 문제를 가질 수 있기 때문에, ` moving average of the cluster centroids in mini-batches`을 사용해서 centroids를 업데이트 한다.   
...
5. **Why are prototypes useful? 분석**
   ...
   1. 아래의 이미지에서 볼 수 있듯이, centroids는 outliers(dicision boundary)에 덜 sensitive하다. 그래서 아래 이미지의 1~4 번 과정이 이뤄지면서 rectify pseudo label이 가능하다. 
...
- **1 해결책**
  ...
  - 그래서 최근 unsupervised learning에서 성공적으로 사용되고 있는, `simultaneously clustering and representation learning`기법을 사용한다. 
  ...
  - 위 필기의 soft, hard는 label을 의미하는 것이 아니라, soft augmentation, hard augmentation을 의미한다. 
...
- **새로운 문제점 2**
  - 위 과정의 장점이 있지만, 단점으로  degeneration issue(one cluster becomes empty = P_k에서 하나만 1이 되고 나머지는 0이라면 모든 feature가 가장 compact하게 뭉치는 것이므로, 의도치 않게 f가 저러한 p를 생성하는 모델이 되어진다.)가 발생한다. 
- **2 해결책**
