...
- **느낀점** :
  ...
  - **실제 공부 순서**    
    ...
    4. 논문 나머지 공부하기
- **목차**
  1. Transformer 최종 정리 이미지
  2. \<Vision Transformer paper\>
  3. \<Vision Transformer - Pytorch code\>
  4. Youtube 발표자료
...
1. Transformers to image recognition 을 수행해보았다. 
   - 지금까지 다른 논문들은, Computer vision에서 self-attention을 사용해다.  self-attention을 하기 위해서 다른 논문들은 inductive biases into the architecture (전체 Network에 attention module을 따로 설치한 행위를 의미하는 듯) 을 적용했지만, 우리는 그러지 않았다. 
   ...
   - classification 과제 (ImageNet, CIFAR-100, VTAB, etc.) 에서 SOTA를 달성했고, 다른 모델에 비해 매우 가볍다. 
2. many challenges 남아있다. 
   - detection and segmentation 으로 나아가야한다. "End-to-end object detection with transformers" 
...
- original Transformer (Vaswani et al., 2017) 를 최대한 따르려고 노력했다. (그래서 이 논문에서는 original Transformer "Attention is all you need." 에 나오는 내용은 추가로 설명하지 않는다. 그래서 이 논문이 어렵다. "Attention is all you need."논문의 내용은 대강 아는데, (논문을 훑어 봐도 [이전 Post](https://junha1125.github.io/blog/artificial-intelligence/2021-01-17-Attention/) 내용 이외에 특별한 내용은 없다. 결국에 코드를 봐서 이해하는게 필요하다.) 그래도 이해가 힘들다. ) 

- 논문에 나오는 내용을 아래와 같이 최종 정리 해놓았다. 이해가 안되는 건, 논문에도 안 써 있다. 결국엔 코드를 봐야한다. 코드를 보자. 

- (코드 추가) 아래 이미지 사이즈에서, h = w = image_size 이며, h=w는 p로 나누어 떨어져야한다. 

- ![image-20210302180259210](C:\Users\sb020\AppData\Roaming\Typora\typora-user-images\image-20210302180259210.png)

- Hybrid Architecture

  1. 위에서는 원본 이미지를 patch로 나눈 후, Embeding을 해서 Transformer에 그대로 넣는 방식를 취했었다. 
  2. 하지만! CNN으로 Feature Map을 일부 뽑아내고, 그렇게 뽑아낸 Feature를 Transformer에 넣는 방식은 어떨까? 이 방식을 사용한 것이 Hybrid Architecture 이다. 
  3. 아래에 실험결과가 나와있지만, 결과는 학습초반에는 좀 더 좋은 성능을 보이다가, 후반에 많은 데이터로 학습 시키고 나서는 1번의 일반 Transformer를 사용한 것이 더 좋은 성능을  보여주었다. 
...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0.):
        ...
        patch_dim = channels * patch_size ** 2  # patch 하나가 가진 pixel의 총 갯수
        ...
        # sin 함수를 이용한 embedding을 하는 줄 알았는데... randn 으로 찾아버린다고?
...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0.):
        ...
        # 8개의 1~h. 1024 vectors -> 64 vectors. qkv에 대해서 각각 1번씩 총 3번 
    ...
    def forward(self, x, mask = None):
        ...
        qkv = self.to_qkv(x).chunk(3, dim = -1) # torch.chunk Tensor를 자른다.
...
![PR-281_ An Image is Worth 16x16 Words_ Transformers for Image Recognition at Scale_210303_110807-04.png](https://github.com/junha1125/Imgaes_For_GitBlog/blob/master/2021-1/16x16_Transformers_for_Image_Recognition/PR-281_%20An%20Image%20is%20Worth%2016x16%20Words_%20Transformers%20for%20Image%20Recognition%20at%20Scale_210303_110807-04.png?raw=true)

![PR-281_ An Image is Worth 16x16 Words_ Transformers for Image Recognition at Scale_210303_110807-05.png](https://github.com/junha1125/Imgaes_For_GitBlog/blob/master/2021-1/16x16_Transformers_for_Image_Recognition/PR-281_%20An%20Image%20is%20Worth%2016x16%20Words_%20Transformers%20for%20Image%20Recognition%20at%20Scale_210303_110807-05.png?raw=true)



![PR-281_ An Image is Worth 16x16 Words_ Transformers for Image Recognition at Scale_210303_110807-16.png](https://github.com/junha1125/Imgaes_For_GitBlog/blob/master/2021-1/16x16_Transformers_for_Image_Recognition/PR-281_%20An%20Image%20is%20Worth%2016x16%20Words_%20Transformers%20for%20Image%20Recognition%20at%20Scale_210303_110807-16.png?raw=true)

![PR-281_ An Image is Worth 16x16 Words_ Transformers for Image Recognition at Scale_210303_110807-17.png](https://github.com/junha1125/Imgaes_For_GitBlog/blob/master/2021-1/16x16_Transformers_for_Image_Recognition/PR-281_%20An%20Image%20is%20Worth%2016x16%20Words_%20Transformers%20for%20Image%20Recognition%20at%20Scale_210303_110807-17.png?raw=true)

![PR-281_ An Image is Worth 16x16 Words_ Transformers for Image Recognition at Scale_210303_110807-20.png](https://github.com/junha1125/Imgaes_For_GitBlog/blob/master/2021-1/16x16_Transformers_for_Image_Recognition/PR-281_%20An%20Image%20is%20Worth%2016x16%20Words_%20Transformers%20for%20Image%20Recognition%20at%20Scale_210303_110807-20.png?raw=true)
