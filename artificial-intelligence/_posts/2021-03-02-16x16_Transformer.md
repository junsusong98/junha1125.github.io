...
- **아래의 코드, 주석에 소중한 내용들이 많이 들어갔으니 꼭 참고하기!!**
...
- 아래의 ` qkv = self.to_qkv(x).chunk(3, dim = -1) `에서 x는 이미지 전체를 1차원 백터(1024개)로 embeding한 값들이 16\*16(256개 patchs)가 모두 모여 있는 백터이다. 즉, 1024 x 256의 x.shape를 가지고 있을 것이다. 
- 자, 그렇다면 256개의 patch 하나하나를 하나의 단어라고 생각하자. 
...
- 같은 원리로 256개의 patch 각각에 대해서, 1개의 Patch는 다른 patch 중 어느 것에 더 집중해서 attention해야하는지를 weight값으로 알려주는 것이 Keys, Querys, Values이고, 이 처리를 모두 하고 나온 백터가 qkv이다.
