...
- 학습 가능한 매개변수(또는 가중치(weight))를 갖는 신경망을 정의합니다.
 < 데이터셋(dataset)을 이용해 아래의 과정을 반복한다. >
1 입력을 신경망에서 전파(process == forward)합니다.
2 손실을 계산합니다.
3 변화도(gradient)를 신경망의 매개변수들에 역으로 전파 -> d_Loss / d_parametar 를 구한다. 
4 신경망의 가중치를 갱신합니다. weight = weight - learning_rate * gradient
"""
pass
...
class Net(nn.Module): # nn.Module 클래스를 상속하여 Net 클래스를 생성한다.
    def __init__(self): # constructor(생성자)에는 함수 매개변수가 없다
    ...
    def forward(self, x): # forward에는 무조건 함수 매개변수로 x가 있어야 한다.
        '''[홈페이지 방법]
    ...
    def num_flat_features(self, x):
        size = x.size()[1:]  # all dimensions except the batch dimension (여기서, the batch dimension = 3차원이면 channel, 4차원이면 이미지 갯수)
        num_features = 1
        for s in size:
            num_features *= s   # 예를 들어 tensor size = (120 c, 6 h, 6 w) 라면, 6*6 = 36 을 return 한다.
        return num_features  
...
# %%
# 1. Sets gradients of all model parameters to zero.
# 2. Backpropa. 1*10 백터를 정규분포에서 랜덤한 값을 가져와 생성한 tensor.  == input이 Net을 통과하고 나오는 tensor의 Size
net.zero_grad()  
out.backward(torch.randn(1, 10)) 
# 하지만 주의하라! 우리가 구해야하는 것은, d_loss/d_x 이다. 따라서 loss에 대한 backward함수를 실행해야한다. 
...
print('conv1.bias.grad after backward')  # d_loss / d_bias_at-conv1
print(net.conv1.bias.grad) 

print("bias 들의 실제 값 = ", net.conv1.bias)

# %% [markdown]
# 위에서 net.conv1.bias 는 그저 tensor이다. (https://pytorch.org/docs/stable/nn.html#conv2d 에서 확인가능)
# 확인가능 conv2d.weight도 사용 가능.)
